...
@PatchMapping("/updateSubject")
    ...
    public String deleteSubject(@AuthenticationPrincipal Student student, @RequestBody @Validated DeleteSubjectPayload body, BindingResult bindingResult){
        if(bindingResult.hasErrors()){
            throw new BadRequestException(bindingResult.getAllErrors());
        }
        return subjectService.deleteSubject(student.getId(), body);
    }
